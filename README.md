...
重画了简洁的结构图，代码中封装了yolo_block的结构，如图中粉色所示，yolo_block的输出，一个流向后方，一个是输出，详细内容可以看代码。
...
$$
\begin{aligned} 
    & \color{red}{\widehat{x}^{(k)} =\frac{x^{(k)}-E[x^{(k)}]}{\sqrt{Var[x^{(k)}]}}} \\[2ex] 
    & \color{red}{y^{(k)} =\gamma^{(k)} \widehat{x}^{(k)}+\beta^{(k)}} \\[2ex]
\end{aligned}
$$
...
![1567321703527](F:/data_study/my_note/0.images/1567321703527.png)
...
![1567319325913](F:/data_study/my_note/0.images/1567319325913.png)
...
![1567344399499](F:/data_study/my_note/0.images/1567344399499.png)
...
$$
![1567346204082](F:/data_study/my_note/0.images/1567346204082.png)
...
![1567346695871](F:/data_study/my_note/0.images/1567346695871.png)
...
$$
\begin{aligned}
& t_{x}=\left(x-x_{a}\right) / w_{a} & t_{y}=\left(x-y_{a}\right) / h_{a} \\[2ex] 
& t_{w}=\log \left(w / w_{a}\right) & t_{h}=\log \left(h / h_{a}\right)\\
\end{aligned}
$$
v2在预测bbox位置参数时采用了强约束方法

![1567400625634](F:/data_study/my_note/0.images/1567400625634.png)

图中黑色虚线框为anchor box，而蓝色框为bbox，预测出的bbox的宽高为(bx,by)和(bw,bh)，计算方式如下
$$
\begin{aligned} 
    & b_x =\sigma(t_x)+c_x \\[2ex] 
    & b_y =\sigma(t_y)+c_y \\[2ex] 
    & b_w =p_w e^{t_w} \\[2ex]  
    & b_h =p_h e^{t_h} \\[2ex] 
\end{aligned}
$$
...
![1567402058839](F:/data_study/my_note/0.images/1567402058839.png)

这一Trick使得网络在不同输入尺寸上都能达到较好到的预测效果 ，是同一网络在不同分辨率上进行检测。大图慢，小图快，总体提高了准确率。因此多尺度训练算是在准确率和速度上达到一个平衡。

![1567402238405](F:/data_study/my_note/0.images/1567402238405.png)
